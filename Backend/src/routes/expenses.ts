...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const actionText =
      expenseStatus === "DRAFT" ? "Draft Created" : "Submitted for Approval";
...
router.patch("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const {
      ...
      receiptUrl,
      ocrData,
      status, // allow promoting from DRAFT → PENDING_APPROVAL
    } = req.body;
...
router.delete("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await prisma.expenseHistory.deleteMany({ where: { expenseId: id } });
